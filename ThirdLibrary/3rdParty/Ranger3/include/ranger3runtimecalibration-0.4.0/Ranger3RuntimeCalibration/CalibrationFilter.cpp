...
#include "3rdparty\\tinyxml2-6.2.0\\tinyxml2.h"
...
{
	if (_mCalibrationPrms.version == 1) {
		applyCalibrationVersion1(inData, outputData, SSEEnabled);
	}
	else if (_mCalibrationPrms.version == 2) {
...
void ranger3calibration::CalibrationFilter::applyCalibrationVersion2(const uint8_t * inData, CalibrationData & outputData, bool SSEEnabled)
{
	...
	const double lens[2 + 4]{ _mCalibrationPrms.opticalCenter[0], _mCalibrationPrms.opticalCenter[1],_mCalibrationPrms.lensDistortion[0],_mCalibrationPrms.lensDistortion[1],_mCalibrationPrms.lensDistortion[2],_mCalibrationPrms.lensDistortion[3] };
	const double scheimpflugTilt{ _mCalibrationPrms.scheimpflugTilt };
	const double scheimpflugRoll{ _mCalibrationPrms.scheimpflugRoll };
	double* Ht = _mCalibrationPrms.H;
...
void CalibrationFilter::applyCalibrationVersion1(const uint8_t *inData, CalibrationData& outputData, bool SSE)
{
	...
	std::transform(_mCalibrationPrms.opticalCenter + 0, _mCalibrationPrms.opticalCenter + 2, stdext::checked_array_iterator<float*>(opticalCenter + 0, 2), [](float a) { return static_cast<float>(a); });
	std::transform(_mCalibrationPrms.lensDistortion + 0, _mCalibrationPrms.lensDistortion + 4, stdext::checked_array_iterator<float*>(lensDistortion + 0, 4), [](float a) { return static_cast<float>(a); });
	std::transform(_mCalibrationPrms.H + 0, _mCalibrationPrms.H + 9, stdext::checked_array_iterator<float*>(H + 0, 9), [](float a) { return static_cast<float>(a); });
	...
	if (SSE && width % 8 == 0) {
		...
		for (int j = 0; j < height; j++) {
			for (int i = 0; i < width / 8; i++) { // We read 8 values at a time.
				...
				for (int k = 0; k < 2; k++) {
					__m128 x = _mm_add_ps(_mm_mul_ps(__m128{(float)i * 8 + 0 + 4 * k, (float)i * 8 + 1 + 4 * k, (float)i * 8 + 2 + 4 * k, (float)i * 8 + 3 + 4 * k}, scaleX_s), offsetX_s);
					__m128 y = _mm_add_ps(_mm_mul_ps(loopData[k], scaleZ_s), offsetZ_s);
...
void CalibrationFilter::calculateWorldTraitsV2() {
	...
	//Setup v2 layout of data.
	double lens[2 + 4];
	std::copy(_mCalibrationPrms.opticalCenter + 0, _mCalibrationPrms.opticalCenter + 2, stdext::checked_array_iterator<double*>(lens + 0, 6));
	std::copy(_mCalibrationPrms.lensDistortion + 0, _mCalibrationPrms.lensDistortion + 4, stdext::checked_array_iterator<double*>(lens + 2, 4));

	CalibrationParameters cpr = _mCalibrationPrms;
	...
	// Initialize with some valid value. 
	float outX, outZ;
	Point2D point = sensorToWorld(Point2D(sensorMinX*scaleX + xOffset, sensorMinZ*scaleZ + zOffset), lens, cpr.scheimpflugRoll, cpr.scheimpflugTilt, cpr.H);
	outX = point.x;
	outZ = point.y;
	float minwr, maxwr, minwx, maxwx;
	minwr = outZ;
	...
	z = sensorMinZ;
	for (x = sensorMinX; x <= sensorMaxX; x++) {
		Point2D point = sensorToWorld(Point2D(x*scaleX + xOffset, z*scaleZ + zOffset), lens, cpr.scheimpflugRoll, cpr.scheimpflugTilt, cpr.H);
		outX = point.x;
		outZ = point.y;
		if (outZ != 0.0f)
		{
...
int CalibrationParameters::loadFromFile(std::string s)
{
	XMLDocument doc;
	XMLError err = doc.LoadFile(s.c_str());
	if (err != XML_SUCCESS) return -1;
	...
	if (*version == '1') {
		...
		readOutHomography(homography, this->H);
		...
		this->opticalCenter[0] = data;

		optcenter->FirstChildElement("V")->QueryDoubleText(&data);
		this->opticalCenter[1] = data;
