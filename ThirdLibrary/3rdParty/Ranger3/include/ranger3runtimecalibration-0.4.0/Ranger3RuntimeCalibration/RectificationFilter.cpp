...
int RectificationFilter::applyRectification(const uint8_t* reflectance, const CalibrationData& calibData, RectificationData& rectData, const RectificationMethod method) {
	if (rectData.range.get() == nullptr) {
		throw std::runtime_error("RectificationData uninitialized.");
	}
	else if (reflectance != nullptr && rectData.reflectance.get() == nullptr) {
	...
	if (reflectance != nullptr) { std::fill(reflectance_out, reflectance_out + rectificationWidth * height, 0.0f); }
				...
				if (xTrans->inRange(*rangeXIn))
				{
					...
					if (method == RectificationMethod::Top || method == RectificationMethod::Bottom) {
						...
						if (isnan(range_it[xi]) || range_it[xi] == _mMissingDataValue || topComp || bottomComp) { // if no value yet or 'better' value
							...
							if (reflectance != nullptr) { refl_it[xi] = static_cast<float>(*reflIn); }
						}
					}
						...
						else if (xi + 1 >= (unsigned long)rectificationWidth)
						{
							...
							if (reflectance != nullptr) { refl_it[rectificationWidth - 1] += w * static_cast<float>(*reflIn); }
						}
						else
						{
							float w = xf - (float)xi;
							...
							if (reflectance != nullptr) {
								refl_it[xi] += wi * static_cast<float>(*reflIn);
								refl_it[xi + 1] += w * static_cast<float>(*reflIn);
							}
						}
					}
				}
